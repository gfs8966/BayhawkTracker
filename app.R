...
library(scales)
library(plotly)
# Version: 2026-U13-field-team 

color_gradient <- colorRampPalette(c("steelblue", "purple"))(14)
training_range <- data.frame(date_only = seq(from = as.Date("2026-03-12"), to = as.Date("2026-05-15"), by = "day"))

# Replace this with your own sheet URL
sheet_url <- "https://docs.google.com/spreadsheets/d/1FWPPod9021rejcp0gyQpy6lgK5gyepp56chS9Kf-nUE/edit?usp=sharing"
gs4_deauth()
...
ui <- fluidPage(
  ...
  titlePanel("U13 Bayhawks Field Lacrosse Workout Tracker"),
  tabsetPanel(
...
server <- function(input, output, session) {
  ...
  sheet_data <- sheet_data %>% 
    ...
    rename(Player = `Select Player`, PushUps = `How many push ups did you complete during the workout?`) %>% 
    mutate(PushUps = suppressWarnings(as.numeric(unlist(PushUps)))) %>% 
    # mutate(Passes = suppressWarnings(as.numeric(unlist(Passes)))) %>% 
    mutate(PushUps = replace_na(PushUps, 0))
  # Barplot output
  ...
  output$pushups <- renderPlotly({
    # Adjust this depending on your data structure
    p2 <- ggplot(sheet_data, aes(y = factor(Player), 
                           x = PushUps, 
                           fill = as.factor(date_only),
                           text = paste0("Player: ", Player,
                                         "<br>Date: ", date_only,
  ...
  team_totals <- sheet_data %>%  
    group_by(date_only) %>%
    summarize(PushUps = sum(PushUps, na.rm = T)) %>% 
    mutate(PushUps = cumsum(PushUps)) %>% 
